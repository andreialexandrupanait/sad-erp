...
return new class extends Migration
{
    ...
    {
        // 1. Create settings_access table if it doesn't exist
        if (!Schema::hasTable('settings_access')) {
        ...
        // Insert default access platforms if table is empty
        if (DB::table('settings_access')->count() === 0) {
        ...
        // 2. Add subscription statuses to settings_subscription if not exist
        if (Schema::hasTable('settings_subscription') && DB::table('settings_subscription')->where('option_type', 'status')->count() === 0) {
        ...
        // 3. Add payment methods to settings_financial if not exist
        // Only insert if an organization exists (settings_financial requires organization_id)
        $orgId = DB::table('organizations')->value('id');
        if ($orgId && Schema::hasTable('settings_financial') && DB::table('settings_financial')->where('option_type', 'payment_method')->count() === 0) {
        ...
        // 4. Drop hierarchical settings tables if they exist
