<x-app-layout>
    <x-slot name="pageTitle">{{ __('Create Custom Field') }}</x-slot>
        ...
        <div class="max-w-3xl mx-auto">
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div class="px-6 py-4 border-b border-slate-200">
                    <h2 class="text-lg font-semibold text-slate-900">{{ __('New Custom Field') }}</h2>
                    <p class="text-sm text-slate-600 mt-1">{{ __('Create a new custom field to capture additional task information.') }}</p>
                </div>

                <!-- Form -->
                <form action="{{ route('task-custom-fields.store') }}" method="POST" class="p-6" x-data="customFieldForm()">
                    @csrf
                    ...
                    <div class="mb-6">
                        ...
                        >
                            ...
                            <option value="date" {{ old('type') === 'date' ? 'selected' : '' }}>{{ __('Date') }}</option>
                            ...
                            <option value="url" {{ old('type') === 'url' ? 'selected' : '' }}>{{ __('URL') }}</option>
                            <option value="phone" {{ old('type') === 'phone' ? 'selected' : '' }}>{{ __('Phone') }}</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
