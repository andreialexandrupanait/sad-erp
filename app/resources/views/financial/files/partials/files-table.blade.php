{{--
    Files Table Partial - Shows file list with bulk actions

    Required variables:
    - $files: Paginated collection of FinancialFile models
    - $year: Selected year
    - $month: Selected month
    - $tip: Selected type (incasare, plata, extrase, general)
--}}

@php
    $monthName = \Carbon\Carbon::create()->setMonth($month)->locale('ro')->isoFormat('MMMM');
    $tipLabels = ['incasare' => 'Încasări', 'plata' => 'Plăți', 'extrase' => 'Extrase', 'general' => 'General'];
    $tipLabel = $tipLabels[$tip] ?? 'General';
...
<div class="bg-white border-b border-slate-200 px-6 py-3">
    <div class="flex items-center gap-2 text-sm">
        ...
        <a href="{{ route('financial.files.index', ['year' => $year, 'month' => $month]) }}"
           class="text-slate-600 hover:text-slate-900 capitalize hover:underline">{{ $monthName }}</a>
        <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
        </svg>
        <span class="text-slate-900 font-medium">{{ $tipLabel }}</span>
    </div>
</div>
                ...
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-100">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Nume fișier</th>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Încărcat la</th>
                            <th class="px-6 py-4 text-right text-xs font-medium text-slate-500 uppercase tracking-wider">Acțiuni</th>
                        </tr>
                    </thead>
