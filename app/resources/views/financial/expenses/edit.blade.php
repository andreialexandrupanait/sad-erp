<x-app-layout>
    <div class="p-6">
        <h1 class="text-2xl font-bold text-slate-900 mb-6">Editează cheltuială</h1>

        <div class="bg-white rounded-lg shadow p-6 max-w-2xl">
            <form method="POST" action="{{ route('financial.cheltuieli.update', $expense) }}">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="document_name" value="{{ old('document_name', $expense->document_name) }}" required
                    ...
                    <div>
                        ...
                        <input type="date" name="occurred_at" value="{{ old('occurred_at', $expense->occurred_at->format('Y-m-d')) }}" required
                               class="w-full rounded-lg border-slate-300">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="note" rows="3" class="w-full rounded-lg border-slate-300">{{ old('note', $expense->note) }}</textarea>
                    </div>
                </div>
