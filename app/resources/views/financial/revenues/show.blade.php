<x-app-layout>
    <x-slot name="pageTitle">Detalii venit</x-slot>

    <x-slot name="headerActions">
        <div class="flex gap-2">
            <x-ui.button variant="outline" onclick="window.location.href='{{ route('financial.revenues.index') }}'">
                <svg class="-ml-1 mr-2 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                </svg>
                {{ __('Back') }}
            </x-ui.button>
            <x-ui.button variant="default" onclick="window.location.href='{{ route('financial.revenues.edit', $revenue) }}'">
                <svg class="-ml-1 mr-2 h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="max-w-4xl mx-auto">
            <x-ui.card>
                <x-ui.card-content>
                    <div class="space-y-6">
                        ...
                        <div class="border-b border-slate-200 pb-4">
                            <h2 class="text-2xl font-bold text-slate-900">{{ $revenue->document_name }}</h2>
                            <p class="text-sm text-slate-500 mt-1">{{ __('Revenue Details') }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-base text-slate-900">{{ $revenue->document_name }}</p>
                            </div>
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-base text-slate-900">{{ $revenue->occurred_at->format('d M Y') }}</p>
                            </div>

                            <!-- Client -->
                            <div>
                                <label class="text-sm font-medium text-slate-500">{{ __('Client') }}</label>
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-base text-slate-900">{{ $revenue->currency }}</p>
                            </div>
                        </div>
                        ...
                        @if($revenue->note)
                            <div class="pt-4 border-t border-slate-200">
                                ...
                                <p class="mt-2 text-base text-slate-700 whitespace-pre-wrap">{{ $revenue->note }}</p>
                            </div>
                        @endif
                        ...
                        @if($revenue->files->isNotEmpty())
                            <div class="pt-4 border-t border-slate-200">
                                <label class="text-sm font-medium text-slate-500 mb-3 block">{{ __('Attached files') }} ({{ $revenue->files->count() }})</label>
                                <div class="space-y-2">
                        ...
                        <div class="pt-4 border-t border-slate-200">
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 text-xs text-slate-500">
                                <div>
                                    ...
                                    <span class="ml-2">{{ $revenue->created_at->format('d M Y, H:i') }}</span>
                                </div>
                                <div>
                                    <span class="font-medium">{{ __('Updated at') }}:</span>
                                    <span class="ml-2">{{ $revenue->updated_at->format('d M Y, H:i') }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex items-center justify-between border-t border-slate-200 px-4 py-4 sm:px-8 bg-slate-50">
                    <form method="POST" action="{{ route('financial.revenues.destroy', $revenue) }}" onsubmit="return confirm('{{ __('Are you sure you want to delete this revenue?') }}')">
                        @csrf
                        @method('DELETE')
