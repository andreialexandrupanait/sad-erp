<x-app-layout>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <x-ui.card>
                        ...
                        <x-ui.card-content class="space-y-4">
                            ...
                            <div class="field-wrapper">
                                ...
                                <div class="mt-2" x-data="{ content: form.introduction || '' }" x-init="
                                    $watch('content', value => form.introduction = value);
                                    $refs.introEditor.addEventListener('trix-change', (e) => content = e.target.innerHTML);
                                    if (form.introduction) { setTimeout(() => $refs.introEditor.editor?.loadHTML(form.introduction), 100); }
                                ">
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        <x-ui.button variant="outline" type="button" onclick="window.location.href='{{ route('offers.show', $offer) }}'" class="w-full justify-center">
    ...
    @push('scripts')
    <script>
    function offerForm(clients, services, selectedClient, offer, existingItems) {
        return {
            // Form data
            form: {
                client_id: offer?.client_id || selectedClient?.id || '',
                title: offer?.title || '',
                valid_until: offer?.valid_until?.split('T')[0] || '{{ date("Y-m-d", strtotime("+30 days")) }}',
            ...
            // Items list (loaded from existing + newly added)
            items: (existingItems && existingItems.length > 0) ? existingItems.map((item, idx) => ({
                _key: Date.now() + idx,
                id: item.id,
