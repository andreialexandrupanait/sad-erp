    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                Edit Subscription: {{ $subscription->vendor_name }}
            </h2>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('subscriptions.update', $subscription) }}" x-data="{ billingCycle: '{{ old('billing_cycle', $subscription->billing_cycle) }}' }">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="mb-4">
                            ...
                            >
                                @foreach(\App\Models\Subscription::billingCycleOptions() as $value => $label)
                                    <option value="{{ $value }}" {{ old('billing_cycle', $subscription->billing_cycle) === $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input
                                ...
                                value="{{ old('next_renewal_date', $subscription->next_renewal_date->format('Y-m-d')) }}"
                                required
                            ...
                            >
                            <p class="mt-1 text-xs text-gray-500">Changing this date will create an audit log entry</p>
                        ...
                        <div class="mb-4">
                            ...
                            >
                                @foreach(\App\Models\Subscription::statusOptions() as $value => $label)
                                    <option value="{{ $value }}" {{ old('status', $subscription->status) === $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
