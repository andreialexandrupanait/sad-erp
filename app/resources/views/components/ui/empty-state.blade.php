...
<div {{ $attributes->merge(['class' => 'flex flex-col items-center justify-center py-12 px-4 text-center']) }} role="status" aria-live="polite">
    <div class="flex h-16 w-16 items-center justify-center rounded-full bg-slate-100 mb-4" aria-hidden="true">
    ...
    @if($description)
        <p class="text-sm text-slate-500 max-w-sm mb-4">{{ $description }}</p>
    @endif

    @if($action || $actionUrl)
        @if($actionUrl)
            <a href="{{ $actionUrl }}" class="inline-flex items-center justify-center rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white shadow hover:bg-slate-800 transition-colors">
                {{ $actionLabel ?? __('Get Started') }}
            </a>
        @else
            {{ $action }}
        @endif
