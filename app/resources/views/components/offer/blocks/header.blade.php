{{-- Header Block - Builder View --}}
{{-- Variables available: block, offer, organization, client, bankAccounts --}}
    ...
    <div class="bg-slate-800 text-white px-10 py-6">
        <div class="flex gap-10">
            ...
            <div class="w-1/2 space-y-1">
                <textarea x-model="block.data.introTitle"
                          x-init="$nextTick(() => { $el.style.height = 'auto'; $el.style.height = $el.scrollHeight + 'px' })"
                          @input="$el.style.height = 'auto'; $el.style.height = $el.scrollHeight + 'px'"
                          placeholder="{{ __('Your business partner for digital solutions.') }}"
                          rows="1"
                          class="w-full text-xl font-bold bg-transparent text-white border-none p-0 focus:ring-0 placeholder:text-slate-400 resize-none overflow-hidden"></textarea>
                <textarea x-model="block.data.introText"
                          x-init="$nextTick(() => { $el.style.height = 'auto'; $el.style.height = $el.scrollHeight + 'px' })"
                          @input="$el.style.height = 'auto'; $el.style.height = $el.scrollHeight + 'px'"
                          placeholder="{{ __('We deliver high-quality services tailored to your specific needs.') }}"
                          rows="1"
                          class="w-full text-sm bg-transparent text-slate-300 border-none p-0 focus:ring-0 placeholder:text-slate-500 resize-none overflow-hidden leading-relaxed"></textarea>
            </div>
